...
export class PlatformViewComponent {
  ...
  ngOnInit(): void {
    this._route.paramMap.subscribe(params => {
      ...
      if (platformIdParam) {
        this.getPlatformById(platformIdParam);
      } else {
  ...
  getPlatformById(id: string): void {
    const foundPlatform = this.platforms.find(platform => platform.id === id);
    this.platform = foundPlatform !== undefined ? foundPlatform : null;
